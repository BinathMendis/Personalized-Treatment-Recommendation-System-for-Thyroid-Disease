...
def verify_otp():
    ...
    if email in otp_storage and otp_storage[email] == int(otp):
        ...
        try:
            ...
            cursor.execute("EXEC SignupUser @Username=?, @Email=?, @Password=?", username, email, password)
            conn.commit()
...
def resend_otp():
    ...
    # Check if the email already exists in otp_storage and delete the old OTP
    if email in otp_storage:
        del otp_storage[email]  # Delete the previous OTP
        
    # Generate and store OTP for the email (use a timestamp for expiry if needed)
    otp = random.randint(100000, 999999)
    otp_storage[email] = otp
    # Simulate sending OTP (e.g., via email or chat service)
    print(f"OTP for {email}: {otp}")  # Replace with your email/sms/chat service integration
...
def reset_password():
    ...
    if email in otp_storage and otp_storage[email] == int(otp):
        ...
        try:
            # Call the ResetPassword stored procedure
